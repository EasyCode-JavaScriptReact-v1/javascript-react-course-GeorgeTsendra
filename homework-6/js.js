 ...
 *
 ...
 3 добавить +1 в счетчик когда встретиться "а"
 ...
 *
    1 разбить на массив по словам
    2 прогнать массив по циклу и разбить на массив каждое слово
    3 развернуть каждое слово и запушить обратно  массив
    4 сделать из массива строку и убрать запятые
 * */

function reverseEachWord(someWord) {
  let newArray = someWord.split(" ");
  // let reverseArray = [];
  let newArrayMap = newArray.map(function(value, index, array) {
    let valueSplit = value.split("");
    let valueSplitReverse = valueSplit.reverse();
    let valueSplitReverseJoin = valueSplitReverse.join()
    return valueSplitReverseJoin.replace(/,/g, "");

  })
  let valueSplitReverseJoinString = newArrayMap.join();

return valueSplitReverseJoinString.replace(/,/g, " ");
}
 ...
 * */
 ...
 function reverseEachWordNew(someWord , bul) {
   let newArray = someWord.split(" ");
   let arrayForEach = [];
   if (bul == true) {
     let newArrayMap = newArray.map(function(value, index, array) {
  ...
  let valueSplitReverseJoinString = newArrayMap.join();
   ...
   return valueSplitReverseJoinString.replace(/,/g, " ");
     ...
     let newMarArray = newArrayMap;
     let newMarArrayReverce =  newMarArray.reverse();
     let newMarArrayReverceJoin = newMarArrayReverce.join();
     return newMarArrayReverceJoin.replace(/,/g, " ");
...
console.log(reverseEachWordNew('You don\'t have to do anything special to begin using the DOM. Different browsers have different implementations of the DOM', false));
 ...
 * а значение это количество повторений
  ...
  1 разделить на массив +
  2 поставить счетчик +
  3 создать обектт +
  4 прогнать по циклу и добавлять в свойство объекта само слово
  5 если слово будет повторяться добавлять счетчик
  6 добавить в обьект свойству количество из счетчика
 * */
...
function wordCounter(sentence) {
 ...
 let arrayForEach = sentenceArray.forEach(function(value, index, arr) {

   let someElem = value;
   obj[value] = sentenceArray.reduce(function (newValue, value, index, arr) {
     if (value == someElem) {
        newValue ++;
     }
     return newValue;
...
function createHashTags(arr) {
  ...
  arr.forEach(function(value, index, arr) {
    ...
    SomeObjeck[value._id] = value.company;
...
function uniqueElements(arr) {
  ...
  let newArrayForReduce = [];
  arr.reduce(function(newValue, value, index, arr) {
    if (value != newValue) {
       newArrayForReduce.push(value);
    }
  },0)
  return newArrayForReduce;
}
